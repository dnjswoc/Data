...
data_file = "서울특별시_행정동별_데이터_업데이트_면적.csv"  # 기존 데이터
df_data = pd.read_csv(data_file, encoding="utf-8-sig")


# 서울특별시 행정동별 면적 데이터 불러오기
seoul_transport = "서울특별시_대중교통_수_업데이트.csv"
df_transport = pd.read_csv(seoul_transport, encoding="utf-8-sig")


# 칼럼 이름 변경
df_transport.columns = ["주소2", "주소3", "면적(km²)"]
df_transport["주소3"] = df_transport["주소3"].str.replace(".", "·")
print(df_transport.head())

# 필요 없는 헤더가 있는 행 제거
df_area = df_transport.iloc[2:].reset_index(drop=True)
